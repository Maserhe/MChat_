...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		caleLayout: function() {
			...
			self.el.bar.style.marginTop = "100px";//顶部距离
			self.el.inner.style.height = withoutSearchHeight;
			var barItemHeight = ((self.el.bar.offsetHeight - 160) / self.el.barItems.length) + 'px';//每个标签行高
			self.el.barItems.forEach(function(item) {
				item.style.height = barItemHeight;
				//item.style.marginTop = "300px";  //每个单元的高
